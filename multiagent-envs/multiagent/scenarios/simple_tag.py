...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, agent_pos=None, check_pos=None, obstacles=None):
        ...
        for i, agent in enumerate(world.agents):  # agent color
            agent.color = np.array(
                [0.35, 0.85, 0.35]) if not agent.adversary else np.array([0.85, 0.35, 0.35])
        # random properties for landmarks
        for i, landmark in enumerate(world.landmarks):
            landmark.color = np.array([0.25, 0.25, 0.25])  # landmark color
        ...
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                landmark.state.p_pos = pos[i]  # landmark
    ...
    def observation(self, agent, world):
        ...
        # return np.concatenate([agent.state.p_pos] + [agent.state.p_vel] + dists)
